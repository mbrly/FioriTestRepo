		...
		onTheMasterPage : {
			...
			actions : {
				...
				iSortTheListOnName : function () {
					return this.iChooseASorter("sortButton", "Sort By <CreatedByUser>");
				},
				iSortTheListOnUnitNumber : function () {
					return this.iChooseASorter("sortButton", "Sort By <TotalNetAmount>");
				},

				iFilterTheListOnUnitNumber : function () {
					return this.iMakeASelection("filterButton", "<TotalNetAmount>", "<100 ");
				},

				iGroupTheList : function () {
					return this.iChooseASorter("groupButton", "<TotalNetAmount> Group");
				},
						...
						success : function (aEntityData) {
							this.waitFor({
								...
								success : function (oList) {
									var sCurrentId,
										aItemsNotInTheList = aEntityData.filter(function (oObject) {
											return !oList.getItems().some(function (oListItem) {
												return oListItem.getBindingContext().getProperty("CustomerReturn") === oObject.CustomerReturn;
											});
										});

									if (!aItemsNotInTheList.length) {
										// Not enough items all of them are displayed so we take the last one
										sCurrentId = aEntityData[aEntityData.length - 1].CustomerReturn;
									} else {
										sCurrentId = aItemsNotInTheList[0].CustomerReturn;
									}
									...
									oCurrentItem.bindingPath = "/" + oList.getModel().createKey("A_CustomerReturn", {
										CustomerReturn : sCurrentId
									});
				...
				iRememberTheListItem : function (oListItem) {
					...
					this.getContext().currentItem = {
						...
						title: oBindingContext.getProperty("CreatedByUser")
					};
				}
			},
			...
			assertions : {
				...
				theListHeaderDisplaysZeroHits : function () {
					return this.waitFor({
						...
						id: "masterPageTitle",
						autoWait: false,
						matchers: new PropertyStrictEquals({name : "text", value : "<A_CustomerReturn> (0)"}),
						success: function () {
				...
				theListShouldBeFilteredOnUnitNumber : function () {
					return this.theListShouldBeFilteredOnFieldUsingComparator("TotalNetAmount", 100);
				},


				theListShouldBeSortedAscendingOnUnitNumber : function () {
					return this.theListShouldBeSortedAscendingOnField("TotalNetAmount");
				},

				theListShouldBeSortedAscendingOnName : function () {
					return this.theListShouldBeSortedAscendingOnField("CreatedByUser");
				},
				...
				theListShouldHaveAllEntries : function () {
					...
					// retrieve all A_CustomerReturn to be able to check for the total amount
					...
					return this.waitFor({
						...
						success : function (oList) {
							...
							this.waitFor({
								id : "masterPageTitle",
								viewName : sViewName,
								matchers : new PropertyStrictEquals({name : "text", value : "<A_CustomerReturn> (" + iExpectedLength + ")"}),
								success : function () {
